...
- 状态：T-002 已完成；D-005 至 D-008 已获 Chat 批准；T-003 独立金标验证进行中
- 审计日期：2026-09-09
- 最后更新：2026-09-10
- 性质：研究/候选，不是正式算法
- 关联任务：T-002、T-003
...
> T-002 的源码、测试、License 与算法差异审计已经完成。D-005 至 D-008 批准的工程和产品边界已同步进入正式规则；其余研究结论仍是候选，不得对外宣称为正式口径。
...
1. 晚子时正式默认采用 `sameDay` 还是 `nextDay`；混合兼容模式已由 D-005 排除。
2. 大运最终采用哪套顺逆与起运折算口径，以及用什么独立来源形成金标。
3. 真太阳时生产算法、经纬度数据源、精度与适用年代。
4. T-003 后续金标的年份范围、节气样本量及专家复核方式。
...
- IANA Time Zone Database 用于历史 UTC offset 与 DST 规则，当前测试数据固定声明 `tzdb-2025b` 为证据基线。
...
- 同日一致候选应保持同日日干推导时干，但尚缺独立权威金标，暂不指定为正式默认。

结论：两种一致候选口径都保留为待决项；默认 `sect=2` 不得直接穿透产品。

### 10.4 真太阳时

第一阶段用 NOAA 近似太阳位置公式验证数据结构及“是否跨时辰”触发逻辑。重庆经度 `106.55°E`、`2024-06-15 01:20 UTC+08:00` 的近似均时差为 `-0.046` 分钟，经度与时区修正后的总偏移约 `-53.846` 分钟，真太阳时候选约 `00:26:09`，从民用丑时跨到子时。

该案例足以验证 D-006 的提示条件，但生产算法、精度、经纬度来源与适用年代仍未批准，因此标记为“暂定计算金标”。
...
已记录 `lunar-javascript@1.7.7` 的可复现候选输出，包括同一出生输入在两种 `sect` 下得到不同精确起运时刻。当前没有独立于候选库、同时明确顺逆与起运折算细节的权威来源，因此这些案例全部标记为“尚未形成金标”，不能判定算法正确。

## 11. 当前证据等级

| 能力 | 第一阶段状态 | 说明 |
|---|---|---|
| 节气事件分钟边界 | 初步通过 | 仅覆盖 2024 立春前后；需扩展 12 个以上节气 |
| IANA 时区、DST gap/fold | 通过 | 可作为输入标准化层金标 |
| 海外与非整点时区 | 通过 | 已覆盖纽约、伦敦、加德满都 |
| 真太阳时跨时辰提示 | 暂定通过 | NOAA 近似公式足以验证触发逻辑；生产算法待定 |
| 立春换年/月柱 | 部分通过 | 库行为已记录；子平口径尚需独立传统来源与纪日金标 |
| 晚子时同日/次日 | 未形成金标 | 混合模式已排除，两个一致候选仍待 Chat 决策 |
| 大运精确起运 | 未形成金标 | 已记录库输出，但缺独立来源 |

## 12. 下一轮审计要求

1. 用 HKO/USNO 或同等级资料扩展至少 12 个节气的分钟边界。
2. 选定独立六十甲子纪日来源，验证日柱与时干，不以第三方代码相互一致替代金标。
3. 为大运顺逆与折算寻找可引用、可复算、明确口径的来源；找不到时继续标记未形成金标。
4. 对未来适配器强制加入：IANA 时区、DST gap/fold、晚子时显式策略、禁止混合模式、候选库版本锁定。

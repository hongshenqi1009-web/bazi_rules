...
- 状态：T-009 已实现真实服务合同 v0.1
- 最后更新：2026-09-15
- 目的：约束真实前后端边界，确保浏览器不复制命理、城市或匹配算法
...
- AI 文案可异步完成，但核心计算结果必须先稳定；AI 超时或失败只令该城市详情变为`unavailable`，不得用样板或未审核模板冒充即时生成结果。
- API 示例是传输合同，不限定 REST、RPC、SSE 或具体框架。
- T-009 实现使用`GET /api/locations`、`POST /api/readings`、`GET /api/readings/:id`和城市内容重试端点；浏览器默认连接这些真实端点。`?demo=1`仅用于明确的本地结构演示，不是生产回退。
...
前端只依据真实 `stage` 切换两段动画，不用假进度条。`content_pending`时可先渲染核心结果；文案未就绪时显示“详细解读暂时不可用，可稍后重试”，不生成假内容。

T-009 为减少服务端保留出生资料，不持久化`input_summary`：页面用当前内存表单生成摘要，刷新或关闭后即消失。上面的`input_summary`字段保留为合同可选项，客户端不得依赖它恢复历史输入。
...
`ranked_cities`必须至少含 3 项，按最终分数降序且由后端稳定处理同分。客户端不得按标签、图片是否完整或本地语言再次排序。T-009 实现把城市内容状态放在`content.status`，枚举为`ready/unavailable`；后者不影响该城市的名次和指数。
...
当`content.status = unavailable`时，三个正文段落可以为空，响应同时提供`message`、`retryable`和`error_code`。前端必须继续展示城市名、指数、标签和已完成的核心结果；如`retryable = true`，可调用`POST /api/readings/:result_id/cities/:city_id/content/retry`。
...
`CONTENT_GENERATION_FAILED`不得使完整推演失败，也不得触发样板/假文案；应把对应城市内容标为`unavailable`并允许稍后重试。
...
- 原始出生输入只存在于浏览器当前页面内存和服务端单次计算闭包；不写数据库、文件、Cookie、浏览器长期存储或应用日志。
- 服务端记录只保存派生结果和审计链，默认在内存保留 15 分钟；进程重启会提前删除。
- AI 内容输入不得包含出生日期、时间、地点、性别、四柱或`result_id`。
- 分享卡与二维码不得包含出生资料或结果 token；二维码目标来自服务端`PUBLIC_APP_URL`配置。
- 免费页在提交前直接展示上述短期处理说明，并提供`/privacy.html`完整说明链接。

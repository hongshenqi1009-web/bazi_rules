...
- 选择经过来源与使用边界审核的城市主图；可为授权摄影或从零生成的正式城市意象图。
...
{
  "asset_id": "",
  "type": "photo | ai_generated_cityscape",
  "source_url": "",
  "creator": "",
  "license": "",
  "attribution": "",
...
城市详情必须使用有地标辨识度、来源清楚的图像。授权照片和从零生成的正式城市意象图都可进入候选，但必须在`type`中明确区分；生成图不得冒充纪实照片，也不能作为城市事实来源。不得使用无法确认城市的泛景、虚构地标或相似城市照片。素材缺失时使用明确的品牌化占位图。
...
- 主图的来源或生成方式、作者/工具、适用许可或条款、哈希、焦点与替代文本完整；
- 不存在把主观氛围写成客观事实的条目。
...
- 当前媒体状态统一为`facts_reviewed_media_candidate`。首批 8 城已绑定从零生成的 1536 × 1024 城市意象母版，包含地标、焦点、替代文本、生成记录与 SHA-256；来源网页的图片、地图与 Logo 仍未进入仓库。
- 无 Profile 的真实 Top 3 仍展示名称、指数、自然标签与排名；详情明确显示暂不可用，不用 demo 文案填充，也不因内容缺失改分。
- 8 城已达到事实闸门和受控构建的媒体候选闸门。公开发布前仍需产品方逐张确认、裁切/压缩 QA，并复核适用平台的 AI 素材披露与商用条款；完成前不称为最终公开媒体验收。

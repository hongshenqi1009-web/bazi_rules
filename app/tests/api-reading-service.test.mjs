...
test("real reading adapter renders server ranking unchanged", async () => {
  ...
  const responses = [
    ...
    jsonResponse({
      ...
      ranked_cities: [
        { rank: 1, city_id: "1", city_name_zh: "甲", city_name_en: "A", compatibility_index: 91, compatibility_tier: "top", very_close_match: true, core_mood_tags: [], thumbnail: {url: "/assets/cities/a-thumb.webp", asset_id: "a-v1"}, hero_media: {url: "/assets/cities/a-detail.webp", asset_id: "a-v1"}, share_media: {url: "/assets/cities/a-share.webp", asset_id: "a-v1"}, content: { status: "unavailable" }, share_line: "甲回应你。" },
  ...
  try {
    ...
    assert.equal(result.rankedCities[0].shareMedia.url, "/assets/cities/a-share.webp");
